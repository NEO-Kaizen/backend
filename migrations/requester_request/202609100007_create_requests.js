exports.up = async function (knex) {
  await knex.schema.createTable('requests', (table) => {
    ...
    table.uuid('requester_id').notNullable().references('requester_id').inTable('requesters').onDelete('RESTRICT');
    ...
    table.integer('category_id').notNullable().references('category_id').inTable('categories').onDelete('RESTRICT');
    table.integer('status_id').notNullable().references('status_id').inTable('statuses').onDelete('RESTRICT');
    table.integer('priority_id').nullable().references('priority_id').inTable('priorities').onDelete('RESTRICT');
    ...
    table.uuid('professional_id').nullable().references('professional_id').inTable('professionals').onDelete('SET NULL');
  ...
  await knex.raw('ALTER TABLE requests ADD CONSTRAINT ck_requests_client_impact CHECK (client_impact IS NULL OR client_impact BETWEEN 1 AND 5)');
