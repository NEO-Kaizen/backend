exports.up = async function (knex) {
  await knex.schema.createTable('attachments', (table) => {
    ...
    table.uuid('request_id').notNullable().references('request_id').inTable('requests').onDelete('RESTRICT');
    table.uuid('pending_item_id').nullable().references('pending_item_id').inTable('pending_items').onDelete('SET NULL');
