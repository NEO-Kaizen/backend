...
- Fonte de verdade: Issue #77 (`issue-metricas-usuarios.md`) e plano de
  execução (`plano-execucao-metricas-usuarios.md`) — define as regras de
  contagem das quatro métricas e a divergência entre o exemplo ilustrativo e
  o resultado real pós-seed.
...
export interface UserMetricsResponse {
  totalUsers: number; // total de usuários cadastrados (todos, ativos ou não)
  activeUsers: number; // usuários com conta ativa (`is_active = true`)
  pendingUsers: number; // usuários com troca de senha pendente
  // (`must_change_password = true` — senha temporária / primeiro acesso)
  adminUsers: number; // usuários vinculados ao perfil `administrador`
...
para `adminUsers`); o resultado é um objeto simples, no formato de contrato
...
```ts
{
  totalUsers: 15,  // number
  activeUsers: 13, // number
  pendingUsers: 3, // number
  adminUsers: 3,   // number
}
```
...
- Valores de `totalUsers`/`activeUsers`/`pendingUsers`/`adminUsers` são
  ...
  projeto.
- `pendingUsers` conta `must_change_password = true` **independentemente de
...
- `activeUsers` representa "contas ativas" (`is_active`), **não** usuários
...
- O filtro de `adminUsers` usa o nome de perfil em minúsculas
...
- O exemplo dos critérios de aceite da Issue #77 indica `adminUsers: 1`
...
- Contagens em banco com o seed de desenvolvimento (users 101–115):
  `totalUsers = 15`, `activeUsers = 13`, `pendingUsers = 3`,
  `adminUsers = 3`.
